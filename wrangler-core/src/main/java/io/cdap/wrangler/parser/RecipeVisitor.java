...
public final class RecipeVisitor extends DirectivesBaseVisitor<RecipeSymbol.Builder> {
  ...
  public RecipeSymbol.Builder visitPropertyList(DirectivesParser.PropertyListContext ctx) {
    ...
    for (DirectivesParser.PropertyContext property : properties) {
      ...
      } else if (property.byteSize() != null) {
        token = new ByteSize(property.byteSize().getText());
      } else if (property.timeDuration() != null) {
        token = new TimeDuration(property.timeDuration().getText());
      } else {
  ...
  /**
   * A Directive can consist of ByteSize field. This visitor method extracts the
   * ByteSize value as <code>Long</code>.
   */
  @Override
   ...
   * the
   * TimeDuration value as <code>Long</code>.
   */
  @Override
  ...
  /**
   * This visitor methods extracts the list of ByteSizes specified. It creates a
   * token
   * type <code>ByteSizeList</code> to be added to <code>TokenGroup</code>.
   */
  @Override
  public RecipeSymbol.Builder visitByteSizeList(DirectivesParser.ByteSizeListContext ctx) {
    List<DirectivesParser.ByteSizeContext> byteSizes = ctx.byteSize();
    List<String> strs = new ArrayList<>();
    for (DirectivesParser.ByteSizeContext byteSize : byteSizes) {
      String text = byteSize.BYTE_SIZE().getText();
      strs.add(text.substring(1, text.length() - 1));
    }
    builder.addToken(new ByteSizeList(strs));
    return builder;
  }

  /**
   * This visitor methods extracts the list of TimeDurations specified. It creates
   * a
   * token
   * type <code>TimeDurationList</code> to be added to <code>TokenGroup</code>.
   */
  @Override
  public RecipeSymbol.Builder visitTimeDurationList(DirectivesParser.TimeDurationListContext ctx) {
    List<DirectivesParser.TimeDurationContext> timeDurations = ctx.timeDuration();
    List<String> strs = new ArrayList<>();
    for (DirectivesParser.TimeDurationContext timeDuration : timeDurations) {
      String text = timeDuration.TIME_DURATION().getText();
      strs.add(text.substring(1, text.length() - 1));
    }
    builder.addToken(new TimeDurationList(strs));
    return builder;
  }
